...
# upstream_unavailable/upstream_busy — агрегатор сам пишет в message
# "Повторите запрос" / "Повторите запрос чуть позже": без этих кодов такой
# ответ (2026-09-06, серия сбоев подряд) падал необрабатываемым AIError с
# первой же попытки, хотя ровно для этого случая и существует
# retry/переключение модели. Оба увидены живьём в один и тот же инцидент —
# похоже, агрегатор различает "недоступен совсем" и "занят, но скоро
# освободится", но для нас разница не важна: оба значат "попробуй ещё раз".
